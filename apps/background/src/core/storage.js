...
export default async () => {
  ...
  if (storedVoice.voice === null) {
    await store.dispatch(settings.setFreeVoice());
  }
  ...
  LocalStorage.getItemsFromStorage(
    ...
    await store.dispatch(settings.set(payload));
  });
  ...
  LocalStorage.getItemsFromStorage(LOCAL_STORAGE_SETTINGS.APP.ITEMS).subscribe(
    async (payload) => {
      ...
      await store.dispatch(app.set(payload));
    }
  );
  ...
  LocalStorage.getItemsFromStorage(
    ...
    await store.dispatch(hotkeys.set(payload));
  });
};
